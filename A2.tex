...
\emph{\underline{Description}}\\
The algorithm establishes a graph with a start and end vertices along with all T$_{i}$ and O$_{i}$, 0 \textgreater i $\leqslant$ n.\\
The start vertex connects to T$_{1}$ with an edge, and O$_{1}$ with another edge, and a weight that represents income of working at T$_{1}$ and O$_{1}$ respectively.\\
