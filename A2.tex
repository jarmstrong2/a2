...
\textbf{Claim}\\
Let G (V, E) be a bipartite graph with vertex partition V = A $\cup$ B, and with edge (u, v), u $\in$ A and v $\in$ B, such that the distance between v and u is at most $s$. Also, at most $t$ edges can be matched to a u. Additionally, let G'(V', E') be the network flow graph constructed based on the algorithm description.\\ 
...
\textbf{(X$\implies$Y)}\\
Each edge (u, v) $\in$ M is one unit flow in G' that is the path S $\rightarrow$ u $\rightarrow$ v $\rightarrow$ T. From max cut flow we know that a cut across (A $\cup$ {S}, B $\cup$ {T}) must equal $\mid$M$\mid$. It follows that the value of flow is $\mid$f$\mid$ = $\mid$M$\mid$.\\
\textbf{(Y$\implies$X)}\\
