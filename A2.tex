...
\textbf{(X$\implies$Y)}\\
Suppose M is a matching of G(V, E). We define f as follows. Begin by gathering all (u, v) $\in$ M, and make sets V$_{i}$ for each hospital v$_{i}$ to contain (u, v) where u $\in$ A and v = v$_{i}$. Then for each (u, v) $\in$ V$_{i}$, then f(S, u) = f (u, v) = 1. For all other edges (u, v) $\in$ E' not in M, let f(u, v) = 0. Then set f(v, T) = $\mid$V$_{i}$. This follows contrainst and flow conservation as outlined in the algorithm. By our definition of f, if we cut across (A $\cup$ {S}, B $\cup$ {T}) then net flow = $\sum_{i=1}^{m}$ V$_{i}$ 

From this f is computed and corresponds to $\mid$M$\mid$\\
