...
\emph{lines 14} is using DAG-SHORTEST-PATHS takes O(n+e) where n is number of vertices, and e is number of edges and since the number of edges e is $\sum_{i = 1}^{n}$ = $\frac{n * (n + 1)}{2}$ therefore it is bounded by O(n$^{2}$). Thus, it is O(n$^{2}$). \\
