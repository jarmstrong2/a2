...
\begin{algorithm}[H]
 \LinesNumbered 
 \KwIn{An nxn matrix R that such that R[i, j] represents the ratio r$_{ij}$, also r$_{ij}$ = 1 where i = j.}
 \KwOut{True if we end up with more of our favourite marbles from some friend x$_{i}$ after initiating a marbling cycle, False otherwise.}
 D$^{(0)} = R$\;
 \For{k: 1 ... n}{
 ...
 \If{i = j and d$_{ij}^{(k)}$ \textgreater ~1}{
 return True\;
 }
 }
 }
...
\emph{Lines 2-11:} O(n$^{3}$) due to triple nested loop.\\
