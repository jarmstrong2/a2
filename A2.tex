...
$\mid$f*$\mid$, max flow, is n, the maximum number of houses. E, the number of edges in G, the constructed bipartite graph is E = (all edges connected to S) + (all edges connected to T) + (all edges between houses and hospitals) = n + m + mn, so E $\leqslant$ O(mn). Therefore
line 21 is O(mn$^{2}$). \\
