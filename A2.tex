...
The definition of the algorithm picks whichever of the two cases is maximum then  d$_{ik}^{(k)}$ is optimal. From this, at every update to d$_{ij}^{(k)}, we check whether a complete marbling cycle, represented by d$_{ij}^{(k)} * r$_{ij}$, is beneficial. If d$_{ij}^{(k)} * r$_{ij}$ > 1 True is returned. Because d$_{ij}^{(k)} is optimal for all k $\leqslqnt$ n, if d$_{ij}^{(k)} * r$_{ij}$ is always less than 1 for all i and j, it follows that a benefical marbling cycles does not exist between the group of n friends.\\
\blacksquare\\
