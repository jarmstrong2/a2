...
\begin{algorithm}[H]
 \LinesNumbered 
 \KwIn{A list of job objects J = \{(t$_{1}$, c$_{1}$), ..., (t$_{k}$, c$_{k}$)\} and a list of people/staff P = \{(p$_{1}$, h$_{1}$, S$_{1}$), ..., (p$_{n}$, h$_{n}$, S$_{n}$)\}}
...
Perform Ford-Fulkerson algorithm using G, assume the function returns some object, call it f. For any edge (u,v) in G, f(u,v) will be the flow through (u,v) where f as a whole represents max flow through G.\\
