...
\textbf{Termination}\\
Termination of the algorithm rests upon the completion of the Ford-Fulkerson procedure. Becuase all capacities are integer values, there will never be any flows with an irrational value. Additionally, Edmonds-Karp algorithm could be used which guarantees termination. It follows that the algorithm must terminate as all other procedures are single iteration or for loops which are guarenteed to complete.\\
$\blacksquare$\\    
