...
The end vertex connects to T$_{n}$ with an edge, and O$_{n}$ with another edge. Both edge weights are 0, as the end will be used in to establish the shortest path.\\
...
It must be kept in mind that before assigning an edge weight we multiply the income value by -1. Because DAG-SHORTEST-PATHS algorithm only find the shortest path, therfore the shortest path in G will also represent the sequence of locations that will give us the most money for n months.\\
Then with the graph we use Short-Path to generate the sequence of jobs that are optimal.\\
