...
\textbf{(X$\implies$Y)}\\
Each edge (u, v) $\in$ M is one unit flow in G' that is the path S $\rightarrow$ u $\rightarrow$ v $\rightarrow$ T. From the Max-flow minicut theorem we know that a cut across (A $\cup$ {S}, B $\cup$ {T}) must equal $\mid$M$\mid$. It follows that the value of flow is $\mid$f$\mid$ = $\mid$M$\mid$.\\
\textbf{(Y$\implies$X)}\\
