...
\begin{cases} 
  ...
  \end{cases}
\]\\
At each update of d$_{ij}^{(k)}$ we perform d$_{ij}^{(k)}$ * r_{ji}. 
If the result is greater than 1 that we have marbling cycle (cycle
 of exchanges) that will be favourable. Thus a True is returned. 
