...
\textbf{Claim}\\
Let G (V, E) be a bipartite graph with vertex partition V = A $\cup$ B, and with edge (u, v), u $\in$ A and v $\in$ B, such that the distance between v and u is at most $s$. Also, at most $t$ edges can be matched to a u. Additionally, let G'(V', E') be the network flow graph constructed based on the algorithm description. If M is a matching of G(V, E) then f is the maximum flow of G'(V', E'), and f = M. Conversely, if  f is the maximum flow of G'(V', E'), then M is a matching of G(V, E), and M = f. \\
% ----------------------------------------------------------------
% Answer ends
% ----------------------------------------------------------------
