...
G.addEdge(start, T$_{1}$, w(start,T$_{1}$) = -(T$_{1}$.pay))\;
G.addEdge(start, O$_{1}$, w(start,O$_{1}$) = -(O$_{1}$.pay))\;
\For{i: 1 ... n - 1}{
G.addEdge(T$_{i}$, T$_{i+1}$, w(T$_{i}$,T$_{i+1}$) = -(T$_{i+1}$.pay))\;
G.addEdge(O$_{i}$, O$_{i+1}$, w(O$_{i}$,O$_{i+1}$) = -(O$_{i+1}$.pay))\;
G.addEdge(T$_{i}$, O$_{i+1}$, w(T$_{i}$,O$_{i+1}$) = -(O$_{i+1}$.pay - move\_fee))\;
G.addEdge(O$_{i}$, T$_{i+1}$, w(O$_{i}$,T$_{i+1}$) = -(O$_{i+1}$.pay - move\_fee))\;
}
G.addEdge(T$_{n}$, end, w(T$_{n}$, end) = T$_{n}$.pay))\;
G.addEdge(O$_{n}$, end, w(O$_{n}$, end) = O$_{n}$.pay))\;
 \# Using the DAG-SHORTEST-PATHS algorithm in CLRS Chap 24.2 \;
DAG-SHORTEST-PATHS(G,w, start)\;
S=[]\;
ShortPath(G, start, end, S=[])\;
return S\;
