...
\textbf{Claim}\\
Suppose given a set A (n houses) and a set B (m hospitals) we compute based on constraints, s, maximum distance from house to hospital, and t, maximum houses a hospital can service, that the all m hospitals can optimally service, in total, k houses. Then it follows that if we construct a graph bipartite graph G(V, E) as described in the description, and execute Ford Fulkerson on G treating it as a network flow, the maximum flow returned will also be k.\\ 
% ----------------------------------------------------------------
% Answer ends
% ----------------------------------------------------------------
