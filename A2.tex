...
\emph{\underline{Running Time}}\\
lines 10-10 are O(n)\\
lines 25 is using DAG-SHORTEST-PATHS takes O(n+e) where n is number of vertices, and e is number of edges and since the number of edges e is 4*(n-1)+4 \textemdash 4 being the edges going out from start and entering end, and 4*(n-1) being the number of edges between T$_{i}$'s and O$_{i}$'s for n months. Thus, it is O(n+n) = O(n). \\
line 27 uses the Short Path algorithm and it takes O(n) for the number of n vertices\\
