...
If the result is greater than 1 then we have marbling cycle (cycle
 ...
 cycle.\\
\emph{Proof}\\
\textbf{Claim}\\ d$_{ij}^{(k)}$ is optimal for all k, and represents that optimal product of exchanges between x$_{i}$ and x$_{j}$ from n set of friends. If the algorithm encounters a benficial marbling cycle (ie. d$_{ij}^{(k)}$ * r$_{ji}$ \textgreater ~1), during any iteration where k is incremented, the algorithm will return True, and False otherwise. 
\textbf{Base Case}\\
