...
G.Start.pay = 0 \;
G.end.pay = 0 \;
G.addEdge(start, T$_{1}$, w(start,T$_{1}$) = 0)\;
G.addEdge(start, O$_{1}$, w(start,O$_{1}$) = 0)\;
\For{i: 1 ... n}{
\If{i \textless n}{

G.addEdge(T$_{i}$, T$_{i+1}$, w(T$_{i}$,T$_{i+1}$) = 0)\;
G.addEdge(O$_{i}$, O$_{i+1}$, w(O$_{i}$,O$_{i+1}$) = 0))\;
G.T$_{i}$.pay = T$_{i}$ * -1 \#initialize the vertex with the negative of their weight \;
G.O$_{i}$.pay = O$_{i}$ * -1 \;
G.addEdge(T$_{i}$, O$_{i+1}$, w(T$_{i}$,O$_{i+1}$) = move\_fee))\;
G.addEdge(O$_{i}$, T$_{i+1}$, w(O$_{i}$,T$_{i+1}$) = move\_fee))\;
}
\Else{
G.addEdge(T$_{i}$, end, w(T$_{i}$, end) = 0))\;
G.addEdge(O$_{i}$, end, w(O$_{i}$, end) = 0))\;}
}
 \# Using the DAG-SHORTEST-PATHS algorithm in CLRS Chap 24.2 \;
DAG-SHORTEST-PATHS(G,w, start)\;
S=[]\;
Income = 0\;
Solution = ShortPath(G, start, end, S=[], Income)\;
return Solution\;
...
\begin{algorithm}[H]
 ...
 \SetKwProg{myprocone}{}{}{}
 \myprocone{\ShortPath{G, start, end, S, Income}}{
\If {end == start  \# when we finish making the path}{
S.add(0, start) \#add the vertex to start of the S sequence\;
return (S, Income*-1)}
\ElseIf{end.parent == NIL \# no path available}{
return [];
}
\Else{
Income += end.pay + w(end.parent, end)\;
S.add(0, end)\;
ShortPath(G,start, end.parent, S, Income)\;
}}
\end{algorithm}
